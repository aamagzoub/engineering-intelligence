...
class SimulationController:
    ...
    def _explain_card_play(self, player_id: int, card, observation) -> str:
        """
        Generate a human-readable explanation for why this card was played.
        Based on game rules and strategy context.
        """
        ...
        has_leading_suit = any(c.suit == leading_suit for c in observation.hand)

        if card.suit == leading_suit:
            # Check if it's the lowest or highest.
            ...
            if card == min(suit_cards_in_hand, key=lambda c: rank_value(c.rank)):
                return f"Playing low — saving high cards"
            elif card == max(suit_cards_in_hand, key=lambda c: rank_value(c.rank)):
                return f"Playing high to win the trick"
            else:
                return f"Playing just enough to win"

        # Void in leading suit.
