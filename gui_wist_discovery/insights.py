"""
Strategic Insights System â AUTO-GENERATED from Q-table patterns.
...
# âââ Pattern Miners (New Minimal Encoding) âââââââââââââââââââââââââââââââââââââââ
...
def _mine_bid_patterns(bid_q, episodes) -> list:
    ...
    # When someone already bid â compete or pass?
    if has_bid_actions:
...
# âââ Dedup & Helpers âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ


def _dedup_merge(insights) -> list:
    """Merge insights by core idea â allow up to 20 per concept."""
    seen = {}
    ...
    for ins in insights:
        ...
        if "ace" in text:
            ...
            # Use more of the text as signature â allows more diversity.
        ...
        if count < MAX_PER_CONCEPT:
            ...
            if text not in [s.get("text", "").lower() for s in seen.values()]:
                ...
                # Exact duplicate â just bump confidence.
                for k, v in seen.items():
                    if v.get("text", "").lower() == text:
                        v["confidence"] = v.get("confidence", 1) + 1
                        break
        else:
            # Over limit â bump confidence on the first one.
...
# âââ Cache Persistence âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
def _mine_deep_patterns(play_items, episodes) -> list:
    ...
    # Insight 3: Action diversity â how decisive is the agent?
    if positive_action_counts and len(positive_action_counts) >= 50:
        ...
        if avg_positives < 2:
            insights.append(_make(
                "In most situations only 1-2 good options exist in most situations, it has developed strong preferences",
                "timing", 1, episodes,
        ...
        if early_qs and late_qs and len(early_qs) >= 10 and len(late_qs) >= 10:
            ...
            if late_avg > early_avg + 0.2:
                insights.append(_make(
                    ...
                    "timing", 1, episodes,
                    why="with fewer unknowns, it's possible to predict outcomes better and chooses more decisively"
                ))
    ...
    # Insight 5: Suit concentration â does one suit dominate across all positions?
...
def _mine_per_suit_rank(play_items, episodes) -> list:
    ...
    for suit in suit_rank_qs:
        ...
        insights.append(_make(
            ...
            "trump", 1, episodes,
            why=f"It turns out that card strength varies by suit, not all suits value the same ranks equally"
        ))
...
def _mine_granular_discoveries(play_items, bid_q, episodes) -> list:
    ...
    dramatically outperforms others â with real numbers and explanations.
    """
    insights = []
    ...
    for n_cards, pos, (rank, suit), low_q, high_q in reversals[:8]:
        ...
        insights.append(_make(
            ...
            "counter-intuitive", 1, episodes,
            why="Discovery: that in this specific situation, playing low wins more than playing high"
        ))
    ...
    for n_cards, pos, best_s, worst_s, spread in suit_dominance_states[:8]:
        ...
        insights.append(_make(
            ...
            "trump", 1, episodes,
            why="Suit choice matters more than rank enormously in this context"
        ))
            ...
            if len(sorted_bids) >= 2:
                ...
                if best_bq - worst_bq > 0.2:
                    insights.append(_make(
                        ...
                        "bidding", 1, episodes,
                        why="Discovery: which bid levels consistently lead to better outcomes"
                    ))
...
def _mine_granular_discoveries(play_items, bid_q, episodes) -> list:
    ...
    for state, actions in play_items:
        ...
        if best_suit != worst_suit and best_rank <= 7 and worst_rank >= 12:
            ...
            desc = _spread_desc(spread)
            text = (f"Trick {trick_num}, leading: leading with {best_name} "
            ...
            cat = "timing"
            why = "Found: this to be the clear best play here"

        insights.append(_make(text, cat, 1, episodes, why=why))
        decisive_count += 1

    # 2. Bid discoveries â specific bid levels that win or lose.
    if bid_q:
        bid_count = 0
        for state, actions in list(bid_q.items())[:2000]:
            ...
            if best_bid == "PASS" and worst_bid.startswith("B"):
                ...
                desc = _spread_desc(spread)
                insights.append(_make(
                    f"Passing {desc} outperforms bidding {wv} in many situations. Defense wins here",
                    "defense", 1, episodes,
                ...
                desc = _spread_desc(spread)
                insights.append(_make(
                    f"Bidding {bv} {desc} outperforms passing. Reliable delivery is possible",
                    "bidding", 1, episodes,
                    why="It turns out to commit when hand shape supports it"
                ))
    ...
    for pos, data in pos_stats.items():
        ...
        if rate > 70:
            ...
            insights.append(_make(
                f"When {pn}, success happens only {rate:.0f}% of the time. Hardest position",
                "timing", 1, episodes,
...
# âââ Snapshot Comparison System ââââââââââââââââââââââââââââââââââââââââââââââââââ
...
def _check_and_take_snapshot(agent) -> list:
    ...
    for interval in _SNAPSHOT_INTERVALS:
        # Which slot are we at? E.g., episodes=150000, interval=50000 â slot 3.
        ...
        if prev_slot_key in snapshots[interval_key]:
            ...
            # Label: e.g., "1.9Mâ2.0M" or "100Kâ150K"
...
def _compare_two_snapshots(prev, current, prev_label, curr_label, interval_label, episodes) -> list:
    ...
    if prev_ranks and curr_ranks:
        ...
        if prev_best != curr_best:
            ...
            insights.append(_make(
                f"Strategy shifted from valuing {prev_name}s most to preferring {curr_name}s, suggesting a change in how it evaluates card strength",
                "counter-intuitive", 1, episodes,
    ...
    if prev_bids and curr_bids:
        ...
        if prev_best_b != curr_best_b:
            if prev_best_b == "PASS" and curr_best_b.startswith("B"):
                ...
                insights.append(_make(
                    f"Bidding confidently works better - preferring to bid {bid_val} rather than passing and defending",
                    "bidding", 1, episodes,
                    why="shifting from passive to active bidding means hand reading became more trusted"
                ))
            elif prev_best_b.startswith("B") and curr_best_b == "PASS":
                insights.append(_make(
                    "Caution pays off - passing and defending rather than commit to a bid",
                    "defense", 1, episodes,
                    why="shifting to defense suggests it turns out that failed bids are costly"
                ))
                ...
                if int(curr_val) > int(prev_val):
                    insights.append(_make(
                        ...
                        "bidding", 1, episodes,
                        why="higher bids indicate winning more tricks became possible than it previously thought"
                    ))
                else:
                    insights.append(_make(
                        ...
                        "bidding", 1, episodes,
                        why="lower bids suggest it turns out to be more realistic about hand strength"
                    ))
    ...
    if curr_ranks and prev_ranks and len(curr_ranks) >= 4 and len(prev_ranks) >= 4:
        ...
        if prev_spread > 0.1 and curr_spread < prev_spread * 0.5:
            insights.append(_make(
                "All card ranks are treated more equally now, meaning context and timing matter more than raw card power",
                "timing", 1, episodes,
                why="converging values show it turns out that WHEN to play matters more than WHAT to play"
            ))
        elif curr_spread > prev_spread * 1.5 and prev_spread > 0.05:
            insights.append(_make(
                "Stronger opinions are forming about which cards are powerful, becoming more decisive in card selection",
                "timing", 1, episodes,
...
# âââ Public Helpers ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
